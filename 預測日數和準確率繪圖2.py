...
# 讀入近期數據
df_L = pd.read_excel('data_Latest.xlsx')
...
def function(num): # 預測日數
    ...
    # 處理 y 資料
    pre_day = num
    df_L[f'Next_{pre_day}Day_Return'] = \
        df_L['Close'].diff(pre_day).shift(-pre_day) # 計算價格變化
        
    df_L['LABEL'] = \
        df_L[f'Next_{pre_day}Day_Return'].apply(
            classify_return) # 創造新的一列 LABEL 來記錄漲跌
...
for i in range(240):
    ...
    if (i + 1) > 1:
        ...
        df_L = df_L.drop(columns = [f'Next_{i}Day_Return'])
    ...
    df_L.to_excel("data_Latest.xlsx", index = False) # 將整理好的資料存成 excel
    
    ones_count = (df_L['LABEL'] == 1).sum()
    zero_count = (df_L['LABEL'] == 0).sum()
    ...
    # 最新資料測試
    X_Latest = df_L[feature_names].values
    y_Latest = df_L['LABEL'].values
    
    latest_data_test_acc = Xgboost.score(X_Latest, y_Latest)
    latest_test_scores.append(latest_data_test_acc)
    
    print(f'Xgboost({i + 1}) 近期數據準確率 %.3f' % latest_data_test_acc)
...
plt.plot(range(1, len(test_list) + 1), test_list,
